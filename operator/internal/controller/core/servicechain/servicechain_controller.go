...
// +kubebuilder:rbac:groups=core.loom.io,resources=servicechains/finalizers,verbs=update
...
func (r *ServiceChainReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !serviceChain.DeletionTimestamp.IsZero() {
		// Handle deletion
		if stringutils.ContainsElement(serviceChain.GetFinalizers(), corev1alpha1.SERVICE_CHAIN_FINALIZER_LABEL) {
			// Remove finalizer
			serviceChain.SetFinalizers(stringutils.RemoveElement(serviceChain.GetFinalizers(), corev1alpha1.SERVICE_CHAIN_FINALIZER_LABEL))
	...
	if !stringutils.ContainsElement(serviceChain.GetFinalizers(), corev1alpha1.SERVICE_CHAIN_FINALIZER_LABEL) {
		serviceChain.SetFinalizers(append(serviceChain.GetFinalizers(), corev1alpha1.SERVICE_CHAIN_FINALIZER_LABEL))
		if err := r.Update(ctx, &serviceChain); err != nil {
			return ctrl.Result{}, err
		}
	}

	var srcApp, dstApp corev1alpha1.Application
	if err := r.Get(ctx, serviceChain.Spec.From.ToNamespacedName(), &srcApp); err != nil {
		if apierrors.IsNotFound(err) {
	...
	if err := r.Get(ctx, serviceChain.Spec.To.ToNamespacedName(), &dstApp); err != nil {
		if apierrors.IsNotFound(err) {
