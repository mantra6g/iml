...
func (r *NetworkFunctionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !nf.DeletionTimestamp.IsZero() {
		// Handle deletion
		if stringutils.ContainsElement(nf.GetFinalizers(), corev1alpha1.NETWORK_FUNCTION_FINALIZER_LABEL) {
			// Remove finalizer
			nf.SetFinalizers(stringutils.RemoveElement(nf.GetFinalizers(), corev1alpha1.NETWORK_FUNCTION_FINALIZER_LABEL))
	...
	if !stringutils.ContainsElement(nf.GetFinalizers(), corev1alpha1.NETWORK_FUNCTION_FINALIZER_LABEL) {
		nf.SetFinalizers(append(nf.GetFinalizers(), corev1alpha1.NETWORK_FUNCTION_FINALIZER_LABEL))
		if err := r.Update(ctx, nf); err != nil {
			return ctrl.Result{}, err
		}
	}
